...
const cellSize = parseInt(
  getComputedStyle(document.documentElement).getPropertyValue("--cellSize")
);
const boardSize = parseInt(
  getComputedStyle(document.documentElement).getPropertyValue("--boardSize")
);
const boardYStart = 0;
const boardXStart = 0;
...
var bombs = 3;
const bombsDisplay = document.querySelector("#bombs");
...
function enterScore() {
  ...
  request.send(
    "user=" + user + "&health=" + health + "&coins=" + coins + "&time=" + time
  );
}
